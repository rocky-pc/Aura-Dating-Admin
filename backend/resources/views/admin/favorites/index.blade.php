...
.u-email{font-size:.74rem;color:var(--txt);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:120px}
...
@media(max-width:1280px){.stat-strip{gap:8px}}
@media(max-width:1024px){.stat-strip{grid-template-columns:repeat(2,1fr)}.fi-search{min-width:140px}}
@media(max-width:860px){.ph{flex-direction:column;align-items:flex-start;gap:10px}.dt thead th:nth-child(3),.dt td:nth-child(3){display:none}}
...
<div class="up">
  ...
  <div class="ph">
    <div>
      <div class="ph-title">Favorites & Swipes</div>
      <div class="ph-sub">View all user interactions, likes, favorites and matches</div>
    </div>
  ...
  <div class="stat-strip">
    <div class="ss ss-cr">
      ...
      <div>
        ...
        <div class="ss-lbl">Total Likes</div>
      </div>
    </div>
    <div class="ss ss-cg">
      <div class="ss-blob"></div>
    ...
    <div class="ss ss-ca">
      ...
      <div class="ss-ico">⭐</div>
      <div>
        <div class="ss-val">{{ number_format($stats['favorites'] ?? 0) }}</div>
        <div class="ss-lbl">Favorites</div>
      </div>
    </div>
    <div class="ss ss-cb">
      <div class="ss-blob"></div>
  ...
  <div class="filter-bar">
    <form method="GET" action="{{ route('admin.favorites.index') }}" class="filter-form">
      ...
      <select name="action" class="fi fi-sel">
        ...
        <option value="favorite"  @selected(request('action')==='favorite')>⭐ Favorites</option>
      ...
      <div class="filter-actions">
        ...
        <a href="{{ route('admin.favorites.index') }}" class="btn-clear">
  ...
  <div class="results-meta">
    <span class="results-count">
      ...
      of <strong>{{ $swipes->total() ?? 0 }}</strong> interactions
    </span>
  </div>
        ...
        <tbody>
          @forelse($swipes as $swipe)
            @php
              ...
              $fromColor = $colors[($swipe->from_user_id ?? 0) % 7];
              $toColor = $colors[($swipe->to_user_id ?? 1) % 7];
              $fromInitials = strtoupper(substr($swipe->fromUser?->email ?? '?', 0, 2));
              $toInitials = strtoupper(substr($swipe->toUser?->email ?? '?', 0, 2));
            @endphp
            <tr>
              <td class="m">{{ $swipe->id }}</td>
              <td>
                <div class="uc">
                  <div class="u-av" style="background:linear-gradient(135deg,{{ $fromColor[0] }},{{ $fromColor[1] }})">{{ $fromInitials }}</div>
                  <div class="u-email">{{ $swipe->fromUser?->email ?? 'Deleted User' }}</div>
                </div>
              </td>
              <td>
                <div class="uc">
                  <div class="u-av" style="background:linear-gradient(135deg,{{ $toColor[0] }},{{ $toColor[1] }})">{{ $toInitials }}</div>
                  <div class="u-email">{{ $swipe->toUser?->email ?? 'Deleted User' }}</div>
                </div>
              </td>
              <td>
              ...
              <td>
                <div class="act-btns">
                  <form method="POST" action="{{ route('admin.favorites.destroy', $swipe->id) }}" style="display:contents" onsubmit="return confirm('Delete this interaction?')">
                    @csrf
                    @method('DELETE')
            ...
            <tr>
              <td colspan="7">
                <div class="empty-state">
                  ...
                  <div class="empty-txt">No interactions match your filters</div>
                </div>
              </td>
            </tr>
          @endforelse
